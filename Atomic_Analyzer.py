...
if option in ['2', 'two']:
    ...
    print()
    shortcut = input(
        "Do you want to calculate the average atomic mass or you want to type? (cal - 1/type - 2)").lower()
    if shortcut == 'cal' or shortcut == '1':
        while True:
            ...
            for values in num_isotopes:
                if values == 1:
                    mass_number = int(
                        input(f"What is the first {labeled}'s mass number? "))
                    questions = [
            ...
            print(
                f"The average atomic mass of {labeled} is {ab_mass} amu ( or {round(ab_mass, 2)} amu)")
            break
    elif shortcut == 'type' or shortcut == '2':
        while True:
            type = float(
                input(f"What is the average atomic mass of {labeled}? "))
    ...
    bond_iso = int(input(
        f"What isotope of {labeled} do you want to calculate the bonding energy? (Enter atomic mass, like if it's 'Bo', enter 11 or 10)  "))
    ...
    try:
        ...
        # The except part catches that error instead of crashing your code.
        delta_mass = type - atomic_mass
    else:
